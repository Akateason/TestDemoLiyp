...
@property (strong, nonatomic) YPFiles  *currentFiles ;
...
- (void)prepareUI {
    ...
    [RACObserve([YPListSortManager sharedInstance], sortOrder) subscribeNext:^(id  _Nullable x) {
        @strongify(self)
        UIImage *img = ![x boolValue] ? [UIImage imageNamed:@"sortDown"] : [UIImage imageNamed:@"sortUp"] ;
    ...
    [YPListCell registerNibFromTable:_table] ;
