...
- (void)configure:(id)model indexPath:(NSIndexPath *)indexPath {
    ...
    if (isSvg) {
        [self.svgImageView startLoadWithUrlStr:aFile.displayThumbNailStr header:kTestTokenHeader] ;
    }
    else {
        [self.imgContent sd_setImageWithURL:[NSURL URLWithString:aFile.displayThumbNailStr]] ;
    }
